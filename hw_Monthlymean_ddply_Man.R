...
for (n=1:nsites){ 
  ...
  data = read.csv(as.character(file_n))
  
  tstr=substr(as.character(file_n),1,12)
  plot(seq(1,10),main=tstr)
  
  plot(seq(1,10),main=as.character(file_n))
...
plot(tmax_anom, x=df[,1], type = "l", col="red", xlab = "", xlim = c(yrmin,yrmax),ylab = expression("Temperature"~degree~C), ylim=c(-1.5,2.5))
